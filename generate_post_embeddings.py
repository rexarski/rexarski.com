#!/usr/bin/env python3
"""
Generate and cache embeddings for Hugo posts using LM Studio.

The script:
* Connects to a running LM Studio server (default: http://127.0.0.1:1234).
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate embeddings for Hugo posts using LM Studio."
    )
    parser.add_argument(
        "--refresh",
        action="store_true",
...
def http_request(path: str, data: Optional[bytes] = None, timeout: float = 60) -> bytes:
    url = f"{LM_STUDIO_HOST.rstrip('/')}{path}"
...
def ping_lm_studio() -> bool:
    ...
    try:
        # Using /models as a generic health check for OpenAI-compatible APIs
        http_request("/models", timeout=3)
        return True
    except Exception:
        return False
...
def embed_text(text: str) -> List[float]:
    """Get embeddings from LM Studio using the OpenAI-compatible endpoint."""
    payload = json.dumps({"model": MODEL_NAME, "input": text}).encode("utf-8")

    raw = http_request("/embeddings", data=payload, timeout=120)
...
def load_database(refresh: bool) -> Dict[str, object]:
    ...
    # If the model changed, we should probably recompute
    if data.get("model") != MODEL_NAME:
        print(f"Model changed from {data.get('model')} to {MODEL_NAME}. Refreshing...")
        return {
            "model": MODEL_NAME,
            "generated_at": datetime.now(timezone.utc).isoformat(),
            "posts": {},
...
def cosine_similarity(vec_a: List[float], vec_b: List[float]) -> float:
    ...
    if len(vec_a) != len(vec_b):
        raise ValueError(
            f"Embedding vectors have different lengths: {len(vec_a)} vs {len(vec_b)}"
        )
...
def compute_similarities(posts: Dict[str, Dict[str, object]]) -> None:
    ...
    for idx, key_a in enumerate(keys):
        ...
        if not isinstance(vec_a, list) or not vec_a:
            # Ensure keys with no embedding still get these fields
            posts[key_a].setdefault("similarities", {})
            posts[key_a].setdefault("top_similar", [])
            continue
        sims: Dict[str, float] = {}
        for key_b in keys:
            if key_a == key_b:
            ...
            if not isinstance(vec_b, list) or not vec_b:
                continue
            score = cosine_similarity(vec_a, vec_b)
            sims[key_b] = score
        posts[key_a]["similarities"] = sims
        sorted_peers = sorted(sims.items(), key=lambda item: item[1], reverse=True)[:3]
...
def main() -> None:
    ...
    if not ping_lm_studio():
        print(f"Error: LM Studio server not found at {LM_STUDIO_HOST}")
    ...
    for key, record in records.items():
        ...
        needs_embedding = (
            args.refresh or embedding is None or cached_hash != record.content_hash
        )
        posts_data[key] = {
            "title": record.title,
            "slug": record.slug,
